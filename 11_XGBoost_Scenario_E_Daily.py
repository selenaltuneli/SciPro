"""Train the daily re-optimization XGBoost model for Scenario E."""
...
OUTPUT_PATH = BASE_DIR / "scenario_E_daily_reopt_forecasts_XGBoost.xlsx"

DATE_COL = "DATE"
TARGET_COL = "WITHDRWLS_ATM"
ATM_COL = "CASHP_ID_ATM"

# Scenario E is re-optimized every day and forecasts the next single day.
REOPT_FREQ_DAYS = 1
HORIZON_DAYS = 1
VALIDATION_DAYS = 60
RANDOM_SEED = 42
...
def run_daily_scenario(df: pd.DataFrame) -> pd.DataFrame:
    feature_columns = get_feature_columns(df)
    schedule = build_reoptimization_schedule(df)
    forecast_parts = []

    for reopt_start in schedule:
        # Scenario E daily version re-optimizes at the end of each day
        # and predicts the next single day.
...
def print_overall_metrics(forecast_df: pd.DataFrame) -> None:
    metrics = compute_metrics(forecast_df)
    print("\nOverall Metrics (Scenario E Daily XGBoost)")
...
def main() -> None:
    ...
    forecast_df = run_daily_scenario(df)
