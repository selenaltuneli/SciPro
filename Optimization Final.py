...
def haversine_km(lat1, lon1, lat2, lon2) -> float:
    ...
    return R*c
# =========================================================
# Helper: ask user for files defenition
# =========================================================
def ask_user_for_input_files():


    selected_files = {
        "atm_master_file": None,
        "edges_file": None,
        "r_file": None,
        "meta_file": None,
    }
    ...
    def pick_file(key, title, filetypes):
        ...
        if path:
            selected_files[key] = path
            labels[key].config(text=os.path.basename(path))

    def submit():
        for name, path in selected_files.items():
            if not path:
                messagebox.showerror("Missing file", f"Please select file for: {name}", parent=root)
                return
            if not os.path.isfile(path):
                messagebox.showerror("Invalid file", f"Selected file does not exist:\n{path}", parent=root)
                return
    ...
    tk.Label(
        root,
        text="Please select all required input files",
        font=("Arial", 13, "bold")
    ).pack(pady=15)
    ...
    row_defs = [
        ("ATM Master CSV", "atm_master_file", [("CSV files", "*.csv"), ("All files", "*.*")]),
        ("Edges Distance CSV", "edges_file", [("CSV files", "*.csv"), ("All files", "*.*")]),
        ("Demand JSON", "r_file", [("JSON files", "*.json"), ("All files", "*.*")]),
        ("Meta JSON", "meta_file", [("JSON files", "*.json"), ("All files", "*.*")]),
    ]

    for idx, (text, key, filetypes) in enumerate(row_defs):
        tk.Label(frame, text=text, width=20, anchor="w").grid(row=idx, column=0, padx=5, pady=8, sticky="w")

        btn = tk.Button(
            frame,
            text=f"Select {text}",
            width=22,
            command=lambda k=key, t=text, ft=filetypes: pick_file(k, f"Select {t}", ft)
        )
        btn.grid(row=idx, column=1, padx=10, pady=8)

        lbl = tk.Label(frame, text="No file selected", width=45, anchor="w", relief="sunken", bg="white")
        lbl.grid(row=idx, column=2, padx=10, pady=8, sticky="w")
        labels[key] = lbl

    button_frame = tk.Frame(root)
    button_frame.pack(pady=20)

    tk.Button(button_frame, text="OK", width=12, command=submit).pack(side="left", padx=10)
    tk.Button(button_frame, text="Cancel", width=12, command=cancel).pack(side="left", padx=10)

    root.mainloop()

    if not selected_files:
        raise FileNotFoundError("File selection was cancelled by the user.")

    return selected_files
# =========================================================
# Answer defenition
# =========================================================
def print_service_summary(model, V0, K, T, W, y, q, p, v, gamma, x=None, A=None, node_to_atm=None, t_to_date=None):
    """
    ...
    for t in T:
        served_rows = []
        date_label = t_to_date.get(str(t), f"(date unknown for t={t})") if t_to_date else f"(t={t})"
        ...
        if not served_rows:
            print(f"t={t} | date={date_label}: (no service)")
            continue

        for (atm_label, k, qval, pval, chosen_w, gam) in served_rows:
            print(
                f"t={t} | date={date_label} | ATM={atm_label} | k={k} | y=1 | "
        ...
        if x is not None and A is not None:
            ...
            for k in K:
                for (i, j) in A:
                    if x[i, j, k, t].X > 1e-6:
                        used_edges += 1
            print(f"t={t} | date={date_label} | used_edges={used_edges}")

    print("=============================================================\n")
def print_daily_routes(model, T, K, A, x, dist, node_to_atm, depot, cost_per_km, t_to_date=None, eps=1e-6):
    """
    ...
    for t in T:
        ...
        for k in K:
            ...
            if not used_edges:
                print(f"\nDay t={t} | date={date_label}, k={k}: (no route)")
                continue

            day_cost = day_km * float(cost_per_km)
            print(f"\nDay t={t} | date={date_label}, k={k}:")
...
# کاهش اسپم خروجی گورو‌بی روی ترمینال
model.Params.OutputFlag = 1 if GUROBI_TO_TERMINAL else 0
if GUROBI_LOGFILE:
...
# =========================================================
# DATA LOADER (USER-SELECTED FILES)
# =========================================================

selected_files = ask_user_for_input_files()

atm_master_file = selected_files["atm_master_file"]
edges_file      = selected_files["edges_file"]
r_file          = selected_files["r_file"]
meta_file       = selected_files["meta_file"]
...
print("Using meta_file      :", meta_file,       datetime.datetime.fromtimestamp(os.path.getmtime(meta_file)))

with open(meta_file, "r", encoding="utf-8") as f:
    meta = json.load(f)

horizon_days = int(meta["horizon_days"])
t_to_date = meta.get("t_to_date", {})

# =========================================================
# RESULT TEXT FILE SETUP
# =========================================================
start_date = t_to_date.get("1", "unknown_start")
end_date   = t_to_date.get(str(horizon_days), "unknown_end")

result_filename = f"optimization result {start_date} to {end_date}.txt"
result_dir = os.path.dirname(r_file) if os.path.dirname(r_file) else "."
...
print(f"Results will also be saved to:\n{result_path}")
print("============================================================\n")
...
V0 = list(range(1, N+1))
V  = [depot] + V0
T = list(range(1, horizon_days + 1))
Tp1 = list(range(1, horizon_days + 2))
...
if model.SolCount == 0:
    log("No feasible solution available. Skipping solution analysis.")
    log("====================================================\n")
...
try:
    status_text = status_map.get(model.Status, f"Status code: {model.Status}")
except Exception:
    status_text = f"Status code: {model.Status}"

log(f"Status: {model.Status} ({status_text})")
log(f"SolCount: {model.SolCount}")

if model.SolCount > 0:
    log(f"Objective: {model.ObjVal:.6f}")

    # ---- optimality diagnostics ----
    log("\n-- Optimality diagnostics --")
    log(f"ObjVal   : {model.ObjVal:.6f}")
    log(f"ObjBound : {model.ObjBound:.6f}")
    log(f"MIPGap   : {model.MIPGap:.10f}")
    log(f"AbsGap   : {abs(model.ObjVal - model.ObjBound):.6f}")
